...
class APITestCase(unittest.TestCase):
    ...
    def test_no_auth(self):
        ...
        self.assertTrue(response.status_code == 200)                # 默认为匿名用户？
    ...
    def test_bad_auth(self):
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers('john@example.com', 'dog'))    # 带请求头
    ...
    def test_token_auth(self):
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers('john@example.com', 'cat'))    # 每个需要权限的访问 都要带验证
        ...
        self.assertTrue(response.status_code == 200)                  # 测试使用token登录

    def test_anonymous(self):
        response = self.client.get(
            url_for('api.get_posts'),
            headers=self.get_api_headers('', ''))
        self.assertTrue(response.status_code == 200)                 # 测试匿名登录
    ...
    def test_posts(self):
        ...
        u = User(email='john@example.com', password='cat', confirmed=True,
                 role=r)
        u2 = User(email='jack@example.com', password='cat', confirmed=True,
                 role=r)
        ...
        response = self.client.post(
            ...
            headers=self.get_api_headers('john@example.com', 'cat'),   # 每次都需要的请求头 用来验证
    ...
    def test_comments(self):
        ...
        # write a comment
        response = self.client.post(    # 该api返回所写评论的url地址
